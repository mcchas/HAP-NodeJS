...
Camera.prototype.handleSnapshotRequest = function(request, callback) {
  ...
  // TPLink NC-250 snapshot URL
  let url = 'http://USER:PASSWORD@IPADDRESS:8080/stream/snapshot.jpg'

  request.get(url)
  .buffer(true).parse(request.parse.image)
  .then(res => callback(undefined, res.body))
